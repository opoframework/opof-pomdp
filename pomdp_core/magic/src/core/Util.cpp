...
vector_t FromRVO(const RVO::Vector2 &v) { return vector_t(v.x(), v.y()); }

float AngleTo(const vector_t &from, const vector_t &to) {
  return RectifyAngle(
      atan2f(from.x * to.y - from.y * to.x, from.x * to.x + from.y * to.y));
}
...
rng_t &RngDet(bool seed, double seed_value) {
  static thread_local rng_t rng(std::random_device{}());
  if (seed) {
    rng = rng_t(static_cast<uint64_t>(
        seed_value *
...
list_t<vector_t> StandardCurveDiscretization(const BezierCurve &curve,
                                             float action_length,
                                             size_t count) {
  ...
  for (size_t i = 0; i < count; i++) {
    float lookahead = t;
    while (lookahead < 1.0f &&
           (curve.Position(lookahead + 1e-5f) - curve.Position(t)).norm() <
               action_length) {
      lookahead += 1e-5f;
    }
    if (lookahead >= 1.0f) {
...
list_t<vector_t> StandardStretchedCurveDiscretization(const BezierCurve &curve,
                                                      float action_length,
                                                      size_t count) {
  ...
  auto get_macro_action = [&](float stretch) {
    return StandardCurveDiscretization(
        BezierCurve(curve.Control0() * stretch, curve.Control1() * stretch,
                    curve.Control2() * stretch, curve.Control3() * stretch),
        action_length, count);
  };
...
bool RectangleContains(const array_t<vector_t, 4> &rect,
                       const vector_t &point) {
  float v1 = (point - rect[0]).dot(rect[1] - rect[0]);
  if (v1 < 0 ||
      v1 > (rect[1] - rect[0])
               .squaredNorm()) { // dot(relative vector, dir / norm(dir)) >=
                                 // norm(dir) iff dot(relative vector, dir) >=
                                 // norm^2(dir)
    return false;
  }
...
bool RectangleIntersects(const array_t<vector_t, 4> &rect1,
                         const array_t<vector_t, 4> &rect2) {
  return RectangleContains(rect1, rect2[0]) ||
         RectangleContains(rect1, rect2[1]) ||
         RectangleContains(rect1, rect2[2]) ||
         RectangleContains(rect1, rect2[3]) ||
         RectangleContains(rect2, rect1[0]) ||
         RectangleContains(rect2, rect1[1]) ||
         RectangleContains(rect2, rect1[2]) ||
         RectangleContains(rect2, rect1[3]);
}

bool ContainsAny(const carla::occupancy::OccupancyMap &area,
                 const array_t<vector_t, 4> &points) {
...
bool ContainsAll(const carla::occupancy::OccupancyMap &area,
                 const array_t<vector_t, 4> &points) {
  for (const vector_t &p : points) {
    if (!area.Contains(p)) {
      return false;
