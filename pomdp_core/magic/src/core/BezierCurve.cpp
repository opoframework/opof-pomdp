...
BezierCurve::BezierCurve(const vector_t &control0, const vector_t &control1,
                         const vector_t &control2, const vector_t &control3)
    : _control0(control0), _control1(control1), _control2(control2),
      _control3(control3) {}
...
vector_t BezierCurve::Position(float t) const {
  if (t <= 0) {
    ...
    return (1 - t) * (1 - t) * (1 - t) * _control0 +
           ...
           3 * (1 - t) * t * t * _control2 + t * t * t * _control3;
  }
}
