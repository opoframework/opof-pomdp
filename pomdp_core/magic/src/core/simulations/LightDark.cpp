...
uint64_t LightDark::Observation::Discretize() const {
  ...
  list_t<int> data{static_cast<int>(floorf(ego_agent_position.x / grid_size)),
                   static_cast<int>(floorf(ego_agent_position.y / grid_size))};
  return boost::hash_value(data);
}

list_t<list_t<LightDark::Action>>
LightDark::Action::CreateHandcrafted(size_t length) {
...
list_t<list_t<LightDark::Action>>
LightDark::Action::Deserialize(const list_t<float> &params,
                               size_t macro_length) {
  list_t<list_t<LightDark::Action>> macro_actions =
      StandardMacroActionDeserialization<LightDark::Action>(params,
                                                            macro_length);
  Action trigger_action;
  trigger_action.trigger = true;
  macro_actions.emplace_back();
...
void LightDark::Action::Encode(list_t<float> &data) const {
  data.emplace_back(trigger ? 1.0 : 0.0);
  data.emplace_back(orientation);
}

void LightDark::Action::Decode(const list_t<float> &data) {
  trigger = static_cast<size_t>(std::lround(data[0])) == 1;
  orientation = data[1];
}
...
LightDark LightDark::CreateRandom() {
  ...
  do {
    EGO_START_MEAN.x = std::uniform_real_distribution<float>(
        RANDOMIZATION_REGION[0].x, RANDOMIZATION_REGION[2].x)(Rng());
    EGO_START_MEAN.y = std::uniform_real_distribution<float>(
        RANDOMIZATION_REGION[0].y, RANDOMIZATION_REGION[2].y)(Rng());
    GOAL.x = std::uniform_real_distribution<float>(
        RANDOMIZATION_REGION[0].x, RANDOMIZATION_REGION[2].x)(Rng());
    GOAL.y = std::uniform_real_distribution<float>(
        RANDOMIZATION_REGION[0].y, RANDOMIZATION_REGION[2].y)(Rng());
    LIGHT_POS = std::uniform_real_distribution<float>(
        RANDOMIZATION_REGION[0].x, RANDOMIZATION_REGION[2].x)(Rng());
  } while ((EGO_START_MEAN - GOAL).norm() < 4 ||
           (std::abs(EGO_START_MEAN.x - LIGHT_POS) < 4 ||
            (std::abs(GOAL.x - LIGHT_POS) < 4)));
...
LightDark LightDark::SampleBeliefPrior() {
  ...
  sim.ego_agent_position.x =
      std::normal_distribution<float>(EGO_START_MEAN.x, EGO_START_STD)(Rng());
  sim.ego_agent_position.y =
      std::normal_distribution<float>(EGO_START_MEAN.y, EGO_START_STD)(Rng());
  return sim;
}
...
// Actually a high probability estimate, assuming actuation noise samples below
// 3 s.d.
float LightDark::BestReward() const {
  ...
  float distance =
      std::max(0.0f, (GOAL - ego_agent_position).norm() - EGO_RADIUS);
...
std::tuple<LightDark, float, LightDark::Observation, float>
LightDark::Step(const LightDark::Action &action,
                const LightDark::Observation *observation) const {
  if (_is_terminal) {
  ...
  if (action.trigger) {
    ...
    next_sim.ego_agent_position +=
        DELTA * vector_t(EGO_SPEED, 0).rotated(action.orientation);
    reward = STEP_REWARD;
    next_sim.step++;
  ...
  if (!observation) {
    if (in_light) {
      ...
      new_observation.ego_agent_position.x +=
          std::normal_distribution<float>(0.0, OBSERVATION_NOISE)(RngDet());
  ...
  if constexpr (compute_log_prob) {
    if (in_light) {
      if (std::isnan(new_observation.ego_agent_position.x) &&
          std::isnan(new_observation.ego_agent_position.y)) {
        log_prob += -std::numeric_limits<float>::infinity();
      } else {
        log_prob +=
            NormalLogProb(next_sim.ego_agent_position.x, OBSERVATION_NOISE,
                          new_observation.ego_agent_position.x);
        log_prob +=
            NormalLogProb(next_sim.ego_agent_position.y, OBSERVATION_NOISE,
                          new_observation.ego_agent_position.y);
      }
    } else {
      if (std::isnan(new_observation.ego_agent_position.x) &&
...
template std::tuple<LightDark, float, LightDark::Observation, float>
LightDark::Step<true>(const LightDark::Action &action,
                      const LightDark::Observation *observation) const;
template std::tuple<LightDark, float, LightDark::Observation, float>
LightDark::Step<false>(const LightDark::Action &action,
                       const LightDark::Observation *observation) const;
...
void LightDark::Decode(const list_t<float> &data) {
...
void LightDark::DecodeContext(const list_t<float> &data) {
  auto iter = data.begin();
...
cv::Mat LightDark::Render(const list_t<LightDark> &belief_sims,
  ...
  auto to_frame = [&](const vector_t &vector) {
    return cv::Point{static_cast<int>((vector.x - SCENARIO_MIN) / RESOLUTION),
  ...
  cv::Mat frame(static_cast<int>((SCENARIO_MAX - SCENARIO_MIN) / RESOLUTION),
  ...
  cv::drawMarker(frame, to_frame(EGO_START_MEAN), cv::Scalar(255, 255, 0),
                 cv::MARKER_TILTED_CROSS, 30, 2, cv::LINE_AA);
  cv::drawMarker(frame, to_frame(GOAL), cv::Scalar(0, 255, 0),
                 cv::MARKER_TILTED_CROSS, 30, 2, cv::LINE_AA);

  for (const simulations::LightDark &belief_sim : belief_sims) {
    cv::drawMarker(frame, to_frame(belief_sim.ego_agent_position),
                   cv::Scalar(0, 255, 255), cv::MARKER_CROSS, 3, 2, cv::LINE_4);
  }
  ...
  const static list_t<cv::Scalar> colors = {
      {75, 25, 230}, {49, 130, 245}, {25, 225, 255}, {240, 240, 70},
  ...
  for (size_t i = 0; i < macro_actions.size(); i++) {
    ...
    for (const Action &a : macro_actions[i]) {
      ...
      if (!a.trigger) {
        cv::line(frame, to_frame(s), to_frame(e), colors[i], 5, cv::LINE_AA);
      }
      s = e;
    }
  }
