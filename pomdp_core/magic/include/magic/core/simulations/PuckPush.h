...
public:
  ...
  static constexpr array_t<vector_t, 4> GOAL_REGION{
      vector_t{980, 125}, vector_t{1167, 125}, vector_t{1167, 475},
      vector_t{980, 475}};

  static constexpr array_t<array_t<vector_t, 2>, 2> NOISY_REGIONS{
      // We consider the expanded regions since the puck cannot properly be
      // observed as long
      // as it is being partially occluded by the paper.
      array_t<vector_t, 2>{vector_t(590 - PUCK_RADIUS, 40 - PUCK_RADIUS),
                           vector_t(700 + PUCK_RADIUS, 558 + PUCK_RADIUS)},
      array_t<vector_t, 2>{vector_t(790 - PUCK_RADIUS, 40 - PUCK_RADIUS),
                           vector_t(900 + PUCK_RADIUS, 558 + PUCK_RADIUS)}};
  static constexpr array_t<vector_t, 4> BOARD_REGION{
      vector_t{37, 40}, vector_t{1250, 40}, vector_t{1250, 558},
      vector_t{37, 558}};
  ...
  struct Action {
    ...
    static list_t<list_t<Action>> Deserialize(const list_t<float> &params,
                                              size_t macro_length);

    void Encode(list_t<float> &data) const;
    void Decode(const list_t<float> &data);
  };
  ...
  std::tuple<PuckPush, float, Observation, float>
  Step(const Action &action, const Observation *observation = nullptr) const;

  // Serialization functions.
  void Encode(list_t<float> &data) const;
  void Decode(const list_t<float> &data);
  static void EncodeContext(list_t<float> &data);
  static void DecodeContext(const list_t<float> &data);
