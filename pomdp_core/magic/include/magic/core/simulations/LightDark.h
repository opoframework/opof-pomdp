...
public:
  ...
  static constexpr array_t<vector_t, 4> RANDOMIZATION_REGION{
      // ego start mean, goal, light pos are bounded within this.
      vector_t{-4, 4}, vector_t{4, 4}, vector_t{4, -4}, vector_t{-4, -4}};
  ...
  struct Action {
    ...
    float Id() const {
      return trigger ? std::numeric_limits<float>::quiet_NaN() : orientation;
    }
  ...
  std::tuple<LightDark, float, Observation, float>
