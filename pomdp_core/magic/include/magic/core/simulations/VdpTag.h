...
public:
  ...
  static constexpr size_t BELIEF_NUM_PARTICLES = 1000;
  static constexpr size_t BELIEF_NUM_THREADS = 1;

  // Planning related parameters.
  static constexpr size_t MAX_STEPS = 50;
  ...
  static constexpr float WORST_REWARD =
      (STEP_REWARD + ACTIVE_MEAS_REWARD) / (1.0f - GAMMA);
  static constexpr size_t SEARCH_DEPTH = 50;
  ...
  std::tuple<VdpTag, float, Observation, float>
  ...
  cv::Mat Render(const list_t<VdpTag> &belief_sims,
