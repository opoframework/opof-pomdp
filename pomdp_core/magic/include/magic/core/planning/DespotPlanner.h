...
public:
  ...
  // Returns: State trajectory, cumulative discounted reward, observation
  // trajectory, log_prob.
  static ExecutionResult
  ExecuteMacroAction(const T &sim,
    ...
    for (; i < std::min(max_steps, macro_action.size()) &&
           !local_sim.IsTerminal();
         i++) {
  ...
  SearchResult
  Search(const TBelief &belief,
         const list_t<list_t<typename T::Action>> &macro_actions) const {
    despot::logging::level(0);
    despot::Globals::config.search_depth = static_cast<int>(lround(
        ceilf(T::SEARCH_DEPTH / static_cast<float>(macro_actions[0].size()))));
    despot::Globals::config.sim_len = despot::Globals::config.search_depth;
    despot::Globals::config.max_policy_sim_len =
        despot::Globals::config.search_depth;
    despot::Globals::config.num_scenarios = T::DESPOT_NUM_SCENARIOS;
    despot::Globals::config.time_per_move =
        static_cast<float>(T::PLANNING_TIME) / 1000;
    despot::Globals::config.discount =
        powf(T::GAMMA, static_cast<float>(macro_actions[0].size()));
    despot::Globals::config.xi = 0.95;

    DespotModel model(&belief, macro_actions);
    despot::DESPOT solver(&model,
...
private:
  ...
  public:
    ...
    list_t<despot::State *> Sample(int num) const override {
      ...
      for (int i = 0; i < num; i++) {
        ...
        if constexpr (!local) {
          ...
          static_cast<DespotState *>(particles.back())->simulation =
              static_cast<const DespotModel *>(model_)
                  ->GetBelief()
                  ->Sample()
                  .CreateLocal();
        }
    ...
    DespotBelief *MakeCopy() const override {
      throw std::logic_error("Unimplemented function.");
    }
  };
  ...
  public:
    double Value(const list_t<despot::State *> &particles,
                 ...
                 despot::History &history) const override {
      double value = 0;
      for (despot::State *particle : particles) {
        value +=
            particle->weight *
            static_cast<double>(
                static_cast<DespotState *>(particle)->simulation.BestReward());
      }
      return value;
    }
  };
  ...
  // IMPORTANT:
  // We always return 0, since we configure DespotDefaultPolicy to always
  // perform a full roll-out to the end of the planning depth. Thus, when stream
  // is exhausted, i.e. the max planning depth has been reached, we return 0
  // since the entire planing horizon has been accounted for (we are ignoring
  ...
  public:
    ...
    despot::ValuedAction
    Value(const list_t<despot::State *> &particles) const override {
  ...
  public:
    DespotDefaultPolicy(const despot::DSPOMDP *model)
        : despot::DefaultPolicy(model,
                                new DespotTrivialParticleLowerBound(model)) {}
  ...
  public:
    DespotModel(const TBelief *belief,
                const list_t<list_t<typename T::Action>> actions)
    ...
    bool Step(despot::State &state, double random_num, despot::ACT_TYPE action,
      ...
      ExecutionResult execution_result =
          ExecuteMacroAction(simulation, _actions[static_cast<size_t>(action)]);
      RngDet(true, std::uniform_real_distribution<double>(0.0, 1.0)(
                       Rng())); // Undeterminize.
      reward = execution_result.reward;

      uint64_t obs_hash = 0;
      for (const typename T::Observation &obs :
           execution_result.observation_trajectory) {
        boost::hash_combine(obs_hash, obs.Discretize());
      }
      obs = obs_hash;

      // obs = execution_result.observation_trajectory.back().Discretize();
      static_cast<DespotState &>(state).simulation =
          execution_result.state_trajectory.back();
      if (static_cast<DespotState &>(state).simulation.IsTerminal()) {
        return true;
      } else {
        return false;
    ...
    double GetMaxReward() const override {
      throw std::logic_error("Unimplemente function.");
    }
    despot::ValuedAction GetBestAction() const override {
      throw std::logic_error("Unimplemented function.");
    }

    despot::ScenarioUpperBound *CreateScenarioUpperBound(
        std::string name = "DEFAULT",
        std::string particle_bound_name = "DEFAULT") const override {
      return new DespotUpperBound();
    }

    despot::ScenarioLowerBound *CreateScenarioLowerBound(
        std::string name = "DEFAULT",
        std::string particle_bound_name = "DEFAULT") const override {
