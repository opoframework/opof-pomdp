...
template <typename T> class Belief {
...
public:
  ...
  Belief ResampleNonTerminal() const {
    ...
    std::uniform_int_distribution<size_t> distribution(
        0, non_terminal_particles.size() - 1);
    for (size_t i = 0; i < _particles.size(); i++) {
  ...
  void Update(const typename T::Action &action,
              const typename T::Observation &observation) {
