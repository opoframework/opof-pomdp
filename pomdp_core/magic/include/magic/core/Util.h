...
#include "magic/core/BezierCurve.h"
...
template <size_t N> array_t<float, N> SoftMax(const array_t<float, N> &logits) {
...
template <typename T> struct list_hash_t {
  std::size_t operator()(const T &l) const {
    return boost::hash_range(l.begin(), l.end());
  }
};

template <typename T> std::string ToBytes(const list_t<T> &list) {
  list_t<char> bytes;
  for (const T &i : list) {
...
template <typename T> list_t<T> FromBytes(const std::string &data) {
  list_t<T> list;
...
template <typename T>
std::tuple<bool, T> FindFirstRootQuadratic(const T &a, const T &b, const T &c,
...
list_t<vector_t> StandardCurveDiscretization(const BezierCurve &curve,
                                             float action_length, size_t count);

// Discretize curve into macro-action of exactly <count> line segments, with
// stretching.
...
template <typename T>
list_t<list_t<typename T::Action>>
StandardMacroActionDeserialization(const list_t<float> &params,
                                   size_t macro_length) {

  if (params.size() != 6 * 8) {
    throw std::logic_error("Expected 48 macro-action parameters, got " +
  ...
  for (size_t i = 0; i * 6 < params.size(); i++) {
    macro_curves.emplace_back(vector_t(0.0f, 0.0f),
  ...
  for (const BezierCurve &curve : macro_curves) {
    list_t<vector_t> macro_action =
        StandardStretchedCurveDiscretization(curve, 1, macro_length);
